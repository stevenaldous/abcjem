...
if ( ! class_exists( 'Understrap_WP_Bootstrap_Navwalker' ) ) {
	...
	class Understrap_WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			// if ( isset( $args->has_children ) && $args->has_children && 1 === $depth && 1 !== $args->depth ) {
			// 	$item_output .= isset( $args->link_before ) ? $args->link_before . $icon_html . strtolower( $title ) . $args->link_after : '';
			// }
			// elseif ( isset( $args->has_children ) && $args->has_children && 0 === $depth && 1 !== $args->depth ) {
			// 	$item_output .= isset( $args->link_before ) ? $args->link_before . $icon_html . strtolower( $title ) . $args->link_after : '';
			// }
			...
			if ( '' !== $linkmod_type ) {
				...
				if ( isset( $args->has_children ) && $args->has_children )  {
					$item_output .= '</a><button' . $attributes . '><span class="sr-only">Toggle Menu</span><i class="fa-regular fa-chevron-down"></i></button>';
				}
				else {
					// With no link mod type set this must be a standard <a> tag.
					$item_output .= '</a>';
