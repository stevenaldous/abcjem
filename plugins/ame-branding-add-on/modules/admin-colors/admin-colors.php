...
class ameBrandingColors extends amePersistentProModule {
	...
	/**
	 * @var null|\YahnisElsts\AdminMenuEditor\Customizable\Settings\BooleanSetting
	 */
	protected $forceEnablePreview = null;
	...
	public function __construct($menuEditor) {
		...
		add_action('admin_menu_editor-register_ac_items', array($this, 'registerAdminCustomizerItems'));
	}
	...
	public function createSettingInstances(ModuleSettings $settings) {
		...
		$schemeSetting->createChild(
			...
			array('maxLength' => 32, 'deleteWhenBlank' => true)
		);
		/**
		...
		foreach (ameAdminColorScheme::getAvailableOptions() as $key => $label) {
			$colorSettings->createChild(
				...
				array('label' => $label, 'deleteWhenBlank' => true)
			);
		}
		...
		foreach ($booleanSettings as $setting => $label) {
			$results[] = $f->boolean($setting, $label);
		}
	...
	public function registerAdminCustomizerItems($adminCustomizer) {
		//This special in-memory setting is used to automatically enable the custom
		//color scheme when the user opens the "Color Scheme" section in the customizer.
		$this->forceEnablePreview = new YahnisElsts\AdminMenuEditor\Customizable\Settings\BooleanSetting(
			'ws_ame_branding-force-enable-preview',
			new LazyArrayStorage(),
			array(
				'default' => false,
				'label'   => 'Preview the color scheme',
			)
		);

		$settings = $this->loadSettings();
		$adminCustomizer->addSettings($settings->getRegisteredSettings());
		$adminCustomizer->addSettings([$this->forceEnablePreview]);
		...
		//Also enable postMessage support for settings that control whether preview is enabled.
		$adminCustomizer->enablePostMessage($this->forceEnablePreview);
		...
		add_action(
			...
			array($this, 'enqueueAdminCustomizerDependencies')
		);
		add_action(
			'admin_menu_editor-enqueue_ac_preview',
			array($this, 'enqueueAdminCustomizerPreview')
		);
	}
	...
	public function enqueueAdminCustomizerDependencies() {
		wp_enqueue_auto_versioned_script(
			...
			array('jquery', 'ame-admin-customizer-js')
		);
	}
	...
	public function enqueueAdminCustomizerPreview() {
		wp_enqueue_auto_versioned_script(
			...
			array('jquery', 'ame-admin-customizer-preview', 'lodash')
		);
		...
		$scriptData = array(
			...
			'forceSettingId'     => $this->forceEnablePreview->getId(),
			'forceEnablePreview' => $this->forceEnablePreview->getValue(),
		);
		...
		foreach ($this->extractColorSettings($this->settings) as $variableName => $setting) {
			$settingData[$setting->getId()] = array(
	...
	public function applyColorScheme() {
		...
		if ( $isPreview ) {
			...
			$stylesheetUrl = add_query_arg(
				array(self::PREVIEW_CACHING_PARAM => 1),
		...
		//Always enable override if forced by the customizer. This makes it possible to preview
		//the color scheme without actually enabling the override for all users.
		if ( $this->forceEnablePreview && $this->forceEnablePreview->getValue() && $isPreview ) {
